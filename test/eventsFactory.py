...
def map_hit_position(hits_df_, local=False):
    ...
    hits_df_["HIT_DRIFT_TIME"] = (
        hits_df_["BX_COUNTER"] + hits_df_["TDC_MEAS"] / 30
    ) * 25 - hits_df_["T0_NS"]

    ref = "LOC" if local else "GLOB"
    hits_df_[f"X_LEFT_{ref}"] = (
        hits_df_[f"WIRE_X_{ref}"] - hits_df_["HIT_DRIFT_TIME"] * VDRIFT
    )
    hits_df_[f"X_RIGHT_{ref}"] = (
        hits_df_[f"WIRE_X_{ref}"] + hits_df_["HIT_DRIFT_TIME"] * VDRIFT
    )
...
def timeShift(hits_df_, cfg, runTimeShift):
    for sl, offset_sl in cfg["time_offset_sl"].items():
        # correction is in the form:
        # coarse offset valid for all SL + fine tuning
        hits_df_.loc[hits_df_["SL"] == sl, "T0_NS"] -= (
            cfg["time_offset_scint"] - runTimeShift + offset_sl
        )  # 6 for 123, 8 for 0
    return
...
def getEvents(stream_df, cfg, runTimeShift, useTrigger):
    ...
    hits_df = stream_df[
        (stream_df.HEAD == cfg["headers"]["valid_hit"]) & (stream_df.TDC_CHANNEL <= 127)
    ].copy()
    ...
    # apply scintillator calibration -> it should be computed
    ...
    hits_df_ = timeShift(hits_df_, cfg, runTimeShift)
